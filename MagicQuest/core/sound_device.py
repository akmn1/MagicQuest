...
def record_audio():
    ...
    recording = []  # 初始化录音数组
    is_recording = False
    samplerate = 16000  # 调整采样率为16000Hz以适应大多数语音识别模型

    def callback(indata, frames, time, status):
        nonlocal recording
        if is_recording:
            # 将数据转换为numpy数组，并确保其具有正确的维度
            indata = np.squeeze(indata).astype(np.int16)  # 确保录音为单声道
    ...
    with sd.InputStream(samplerate=samplerate, channels=1, dtype='int16', callback=callback):  # 将声道数改为1
        def on_press(event):
...
def save_recording(recording):
    ...
    write(temp_file.name, 16000, recording)  # 保存文件时明确指定采样率为16000Hz
